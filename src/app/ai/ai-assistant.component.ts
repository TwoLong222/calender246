    ...
    this.ai.chat(text, history).subscribe({
      next: (res) => {
        ...
        if (res.intent === 'search_events') {
          ...
          // Cấu trúc nhiều dòng của nhánh hieu (để chèn thêm dòng ngày lễ), nhưng câu
          // "không tìm thấy" dùng khóa dịch, không thì chế độ tiếng Anh lòi ra tiếng Việt.
          const lines = [res.reply, found.length ? this.listMsg(found) : `(${this.tr.t('ai.err.noEvents')})`];
